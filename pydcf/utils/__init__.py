...
def _get_timeseries(infile1, infile2, vrbs, plyft):
    ...
    ts1_in = np_loadtxt(infile1, comments='#', delimiter=',')
    ts2_in = np_loadtxt(infile2, comments='#', delimiter=',')

    ts1 = chck_tserr(ts1_in)
    ts2 = chck_tserr(ts2_in)

    ts1, ts2 = _set_unitytime(ts1, ts2)
